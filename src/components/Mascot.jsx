...
const messages = {
  welcome: { fr: 'Bonjour ! On apprend ensemble ?', es: '¡Hola! ¿Aprendemos juntos?' },
...
export default function Mascot({ message = 'welcome', size = 'md', animate = true }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className={`${sizes[size]} relative ${animate ? 'animate-bounce-in' : ''}`}>
        <svg viewBox="0 0 120 120" className="w-full h-full">
          ...
          {/* Mouth */}
          <path d="M 52 48 Q 60 54 68 48" fill="none" stroke="#1a1a1a" strokeWidth="1.5" />
          {/* Cheeks */}
          <circle cx="40" cy="45" r="5" fill="#FFB5B5" opacity="0.6" />
          <circle cx="80" cy="45" r="5" fill="#FFB5B5" opacity="0.6" />
          {/* Arms */}
          <g className={isWaving ? 'origin-[85px_60px] animate-[wave_0.5s_ease-in-out_3]' : ''}>
            <ellipse cx="28" cy="68" rx="8" ry="14" fill="#8B6F47" transform="rotate(-15 28 68)" />
          </g>
          <ellipse cx="92" cy="68" rx="8" ry="14" fill="#8B6F47" transform="rotate(15 92 68)" />
          {/* Feet */}
      ...
      {currentMsg && (
        <div className="bg-white rounded-2xl px-4 py-2 shadow-md border border-gray-100 max-w-[200px] text-center relative animate-pop">
          ...
          <p className="font-display text-xs font-bold text-gray-700 relative z-10">{currentMsg.fr}</p>
          <p className="font-body text-[10px] text-gray-400 relative z-10">{currentMsg.es}</p>
        </div>
      )}
    </div>
  );
