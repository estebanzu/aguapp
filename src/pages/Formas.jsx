...
function FormaSVG({ forma, size = 80, active = false }) {
  ...
  switch (forma.svg) {
    ...
    case 'square':
      return (
        <svg width={size} height={size} viewBox="0 0 100 100">
          <rect x="10" y="10" width="80" height="80" rx="4" fill={fill} stroke={stroke} strokeWidth={sw} />
        </svg>
      );
    case 'triangle':
      return (
        <svg width={size} height={size} viewBox="0 0 100 100">
          <polygon points="50,8 92,92 8,92" fill={fill} stroke={stroke} strokeWidth={sw} />
        </svg>
      );
    case 'rectangle':
      return (
        <svg width={size} height={size} viewBox="0 0 100 100">
          <rect x="5" y="20" width="90" height="60" rx="4" fill={fill} stroke={stroke} strokeWidth={sw} />
        </svg>
      );
    case 'star':
      return (
...
export default function Formas() {
  ...
  const renderFormaOption = (forma, { isCorrect, isSelectedWrong, onSelect, disabled }) => (
    <button
      onClick={onSelect}
      disabled={disabled}
      className={`
    ...
    >
      ...
      <span className="font-display font-bold text-sm text-gray-700">{forma.fr}</span>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl font-display font-bold text-sm transition-all ${
              mode === 'explore' ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            🔍 Explorer
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-xl font-display font-bold text-sm transition-all ${
              mode === 'quiz' ? 'bg-yellow-400 text-gray-800' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            ⭐ Jouer
          </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-5 max-w-lg mx-auto">
            {formas.map((forma) => (
              <button
                ...
                `}
              >
                <FormaSVG forma={forma} size={70} active={activeForma === forma.id} />
                <span className="font-display font-bold text-gray-700">{forma.fr}</span>
                <span className="font-body text-xs text-gray-400">{forma.es}</span>
              </button>
            ))}
          </div>
